...
| `ovos.intent.list` | * → core | Query the intent manifest (INTENT-4 §10); optional filters `skill_id`, `lang`, `session_id`; `lang` resolves per intent like utterance matching (exact, else the closest registered region of the same language, e.g. `en-CA` → `en-US`; rows keep their registered `lang`); `include_definitions: true` attaches each row's registration payload |
...
| `ovos.intent.describe` | * → core | Fetch the registration payload(s) behind one intent: `skill_id`, `intent_name`, `lang` (optional `method`); `lang` falls back to the closest registered region of the same language, like `ovos.intent.list` |
